 ...
 * Also verifies AddressBookPage::Filter::Signable shows only signable addresses.
 */
void TestSignableAddressFilter(interfaces::Node& node)
{
...

    {
        ...
        WalletDescriptor legacy_w_desc(std::move(legacy_descs[0]), 0, 0, 1, 1);
    ...
    // Filter::Signable must show only the spendable PKHash; no filter shows all
    ...
    AddressBookPage page_signable{platformStyle.get(), AddressBookPage::ForSelection, AddressBookPage::ReceivingTab,
                                  nullptr, AddressBookPage::Filter::Signable};
